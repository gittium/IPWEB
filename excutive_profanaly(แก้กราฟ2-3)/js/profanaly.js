...
document.addEventListener("DOMContentLoaded", function () {
    Promise.all([
        ...
        loadRewardOptions(),
        loadTeacherOptions(),
        loadTopPaidJobPostsCategoryOptions()  // โหลดตัวเลือกสำหรับ topPaidJobPosts
        // ถ้ามีฟังก์ชันสำหรับ topPaidJobPosts_status, _reward, _teacher ก็เพิ่มเข้ามาเช่นกัน
    ]).then(() => {
        ...
        .then(data => {
            ...
            if (categorySelect) {
                ...
                data.forEach(item => {
                    ...
                    opt.value = item.job_category_id;        // เปลี่ยนจาก job_categories_id
                    opt.textContent = item.job_category_name;  // เปลี่ยนจาก categories_name
                    categorySelect.appendChild(opt);
                });
            }
            // For individual filters (selectsที่มี id ลงท้ายด้วย _category)
        ...
        .then(data => {
            ...
            if (rewardSelect) {
                ...
                data.forEach(item => {
                    ...
                    opt.textContent = item.reward_type_name;  // เปลี่ยนจาก reward_name
    ...
    return fetch("api/api.php?endpoint=teacher-list")
        ...
        .then(data => {
            ...
            if (teacherSelect) {
                ...
                data.forEach(item => {
                    const opt = document.createElement("option");
                    opt.value = item.teacher_id;     // เปลี่ยนจาก teachers_id
                    opt.textContent = item.teach_name; // เปลี่ยนจาก name
