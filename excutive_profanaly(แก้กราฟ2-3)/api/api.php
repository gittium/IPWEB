...
try {
    ...
    ";
        ...
        */
        ...
        case 'top-paid-jobs':
            $sql = "
                    SELECT pj.title, MAX(asl.salary) AS salary
                    FROM post_job pj
                    JOIN accepted_student asl ON pj.post_job_id = asl.post_job_id
                    WHERE asl.salary IS NOT NULL
                      AND pj.reward_type_id = 2
                      AND pj.job_status_id = 2
                ";
            if (!empty($whereClause)) {
                $sql .= " AND " . ltrim($whereClause, "WHERE ");
            }
            $sql .= "
                    GROUP BY pj.post_job_id, pj.title
