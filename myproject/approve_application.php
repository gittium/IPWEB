    ...
    if ($stmt->execute()) {
        ...
        if ($stmt_insert_student->execute()) {
            ...
            if ($check_data['accepted_count'] >= $number_student) {
                ...
                // ทำการ auto-reject ใบสมัครที่ยังเหลืออยู่
                $sql_reject_others = "INSERT INTO accepted_application 
                    (job_application_id, post_job_id, student_id, accept_status_id, created_at)
                    SELECT ja.job_application_id, ja.post_job_id, ja.student_id, 2, NOW()
                    FROM job_application ja
                    LEFT JOIN accepted_application aa ON ja.job_application_id = aa.job_application_id
                    WHERE ja.post_job_id = ? AND aa.job_application_id IS NULL";
                $stmt_reject = $conn->prepare($sql_reject_others);
                $stmt_reject->bind_param("i", $post_id);
                $stmt_reject->execute();

                // ส่งการแจ้งเตือนให้นิสิตที่ถูก auto-reject ด้วย event_type 'job_application'
                foreach ($pending_applications as $pending) {
                    $auto_student_id = $pending['student_id'];
                    // สามารถระบุ reference_id เป็น 0 หากไม่ได้ใช้อ้างอิง accepted_application
                    $reference_id = 0;
                    $notification_message = "❌ คุณถูกปฏิเสธจากงาน '$job_title' ";
                    $event_type_auto = 'job_application';
                    $role_id_auto = 4; // สำหรับผู้สมัคร
                    $reference_table_auto = 'accepted_application';
                    $status_auto = 'unread';

                    $sql_notify_auto = "INSERT INTO notification (user_id, role_id, event_type, reference_table, reference_id, message, status, created_at) 
                                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                    $stmt_notify_auto = $conn->prepare($sql_notify_auto);
                    $stmt_notify_auto->bind_param("sississs", $auto_student_id, $role_id_auto, $event_type_auto, $reference_table_auto, $reference_id, $notification_message, $status_auto, $created_at);
