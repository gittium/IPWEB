...
$teachers_id = isset($_GET['teachers_id']) ? strval($_GET['teachers_id']) : null;
...
<body>
    ...
    const teachers_id = urlParams.get('teachers_id');

    // Fetch jobs data
    fetch(`?action=get_jobs&teachers_id=${teachers_id}`)
        .then(res => res.json())
        .then(data => {
    ...
    // Fetch students when a job is selected
    jobSelect.addEventListener("change", function() {
        ...
        fetch(`?action=get_students&teachers_id=${teachers_id}&post_jobs_id=${jobSelect.value}`)
            .then(res => res.json())
            .then(data => {
