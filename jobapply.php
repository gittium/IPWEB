...
$post_jobs_id = isset($_GET['id']) ? $_GET['id'] : 1;
...
$sql_check = "SELECT teachers_id FROM post_jobs WHERE post_jobs_id = ?";
...
if ($row = $result_check->fetch_assoc()) {
    $teacher_id = $row['teachers_id']; // ดึงค่า teacher_id
} else {
...
// ดึงข้อมูลนักศึกษา
$sql = "SELECT post_jobs.title, students.name, students.year, students.email, major.major_name 
        FROM students 
        JOIN major ON students.major_id = major.major_id 
        JOIN post_jobs ON post_jobs.post_jobs_id = ?
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_FILES["resume"]["name"])) {
        ...
        if (move_uploaded_file($_FILES["resume"]["tmp_name"], $resume_path)) {
            ...
            $sql_insert = "INSERT INTO job_application (students_id, post_jobs_id, GPA, phone_number, resume) VALUES (?, ?, ?, ?, ?)";
            ...
            if ($stmt_insert->execute()) {
                ...
                $sql_notify = "INSERT INTO notification (user_id, roles_id, event_type, reference_table, reference_id, message, status) 
                VALUES (?, ?, ?, ?,?, ?, ?)";
                $stmt_notify = $conn->prepare($sql_notify);
                $stmt_notify->bind_param("iississ", $students_id, $role_id, $event_type, $reference_table, $reference_id, $message, $status);
