...
function renderPieChart(canvasId, labels, values, colorsArray, chartTitle = "") {
    ...
    // ตรวจสอบกราฟที่เป็น Job Status Distribution และ Job Categories Distribution
    const isJobStatusChart = canvasId === "jobStatusChart";
    const isJobCategoriesChart = canvasId === "jobCategoriesChart";
    const isRewardTypeChart = canvasId === "rewardTypeChart"; // หากมีใช้งาน

    // สำหรับกราฟที่เป็น Job Status Distribution และ Job Categories Distribution ให้แสดงเฉพาะเปอร์เซ็นต์
    if (isJobStatusChart || isJobCategoriesChart || isRewardTypeChart) {
        chartInstances[canvasId] = new Chart(document.getElementById(canvasId), {
            type: "pie",
            data: {
                labels,
                ...
                plugins: {
                    ...
                    datalabels: {
                        ...
                        formatter: function (value, context) {
                            ...
                            // ถ้าเป็นกราฟ Job Status Distribution และค่าเป็น 0.0% ให้ return เป็นค่าว่าง
                            if (isJobStatusChart && percent === "0.0") {
                                return "";
                            }
                            return percent + '%';
                        },
        ...
        // สำหรับกราฟอื่น ๆ ใช้โค้ดเดิมที่แสดงทั้งชื่อและเปอร์เซ็นต์
        chartInstances[canvasId] = new Chart(document.getElementById(canvasId), {
            type: "pie",
            data: {
                labels,
