...
/* ----------------------------------------------------------------
   2. การบันทึกข้อมูลรีวิว (POST)
      - รับข้อมูล: students_id, post_job_id, คะแนนในแต่ละหมวด และคอมเมนต์สำหรับหมวดที่ไม่มีคะแนน (เช่น review_category_id = 6)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // สำหรับหมวดที่ไม่มีคะแนน (สมมุติว่า review_category_id = 6 คือหมวดความคิดเห็น)
    ...
    foreach ($categories as $category) {
        ...
        // ถ้าเป็นหมวดที่ไม่มีคะแนน (เช่น id 6) ให้ตั้ง rating เป็น 0 และใช้คอมเมนต์ที่รับมา
        if ($review_category_id == 6) {
            $rating = 0;
            ...
            fetch("?action=get_categories")
                ...
                .then(categories => {
                    ...
                    categories.forEach(cat => {
                        ...
                        // หากหมวดรีวิว id = 6 ให้ใช้ textarea สำหรับคอมเมนต์
                        if (cat.review_category_id == 6) {
                            div.innerHTML = `
